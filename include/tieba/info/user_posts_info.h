...
namespace tieba {
	...
	inline UserPostsInfo get_user_posts_info(uint uid, int resNum = 25, bool needContent = true, uint64 endTime = 0, int orderType = 1, int offset = 0)
	{
		UserPostsInfo info;
		string url = !endTime ?
			(boost::format("http://service.tieba.baidu.com/service/post?method=queryUserPost&format=json&input={\"user_id\":%1%,\"res_num\":%2%,\"need_content\":%3%,\"order_type\":%4%,\"offset\":%5%,\"delete_type\":2,\"ueg_ip\":1}") % uid % resNum % needContent % orderType % offset).str() :
			(boost::format("http://service.tieba.baidu.com/service/post?method=queryUserPost&format=json&input={\"user_id\":%1%,\"res_num\":%2%,\"need_content\":%3%,\"order_type\":%4%,\"offset\":%5%,\"delete_type\":2,\"ueg_ip\":1,\"end_time\":%6%}") % uid % resNum % needContent % orderType % offset % endTime).str();
