...
#include <iostream>
#include "gbk_complex_simpify_table.h" //for the source table
...
namespace ch_convert
{
	...
	public:
		...
		ChConverter()
		{
			loadChineseTable(); //默认通过直接载入繁简体对照
		};

		ChConverter(const char* strFile)
		{
	...
	public:
		typedef unordered_map<std::string, std::string>  CHARACTER_TABLE;
		bool loadChineseTable()
		{
			int words_size = sizeof(ComplexFonts) / sizeof(char*);
			int words_size2 = sizeof(SimplifiedFonts) / sizeof(char*);
			//        assert(words_size == words_size2);
			string complex_key, sim_key;
			for (int i = 0; i < words_size; i++)
			{
				gconv::_UTF8_to_GBK_(ComplexFonts[i], complex_key);
				gconv::_UTF8_to_GBK_(SimplifiedFonts[i], sim_key);
				m_hashChinese[complex_key] = sim_key;
			}
			return true;
		}
		...
		bool loadChineseTable(const char * strFile)
		{
	...
	typedef ChConverter CharacterConverter;
