...
Value::asString(bool isGBK, const std::string& defaultStr) const
{
  ...
  switch (type_) {
    ...
    case stringValue:
      if (!isGBK)
        return value_.string_ ? value_.string_ : defaultStr;
      else
        return value_.string_ ? gezi::to_gbk(value_.string_) : defaultStr;
    case booleanValue:
      return STRING(value_.bool_);
    case intValue:
      return STRING(value_.int_);
    case uintValue:
      return STRING(value_.uint_);
    case realValue:
...
//TODO glog «∑Ò÷ß≥÷LOG_IF(WARNING, ...) << throw(1);  test
Value
