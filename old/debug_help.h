...
#define Pval(s)\
    cout << "[" << __LINE__ << ":" << __FUNCTION__ << "]: " << #s <<" ------- " << s << endl
#define Pval2(s)\
    cout << #s <<" ------- " << endl << s << endl
 ...
 */
template<typename T>
void Pvec(const T&vec, std::ostream& out = std::cout, const string& sep = " | ", const string& last = "\n")
{
...
template<typename T>
void Pvec2(const T&vec, std::ostream& out = std::cout, const string& sep = " | ", const string& last = "\n")
{
    typedef typename T::value_type VType;
    typedef typename T::const_iterator Iter;
    for (Iter iter = vec.begin(); iter != vec.end(); ++iter)
...
void Prange2(Iter begin, Iter end, std::ostream& out = std::cout, const string& sep = " | ", const string& last = "\n")
{
    for (; begin != end; ++begin)
    {
...
template<typename Iter>
void Prange(Iter begin, Iter end, std::ostream& out = std::cout, const string& sep = " | ", const string& last = "\n")
{
    for (; begin != end; ++begin)
    {
        out << *begin << sep;
