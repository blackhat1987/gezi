...
template<typename _KeyIDMap>
class Printer
{
public:
    Printer(const _KeyIDMap& keyid_map)
    ...
    template<typename _Iter>
    void print_post2(_Iter iter_s, _Iter iter_e, std::ostream& os = std::cout, const string& sep = " | ")
    {
        _Iter begin = iter_s;
        for (; iter_s != iter_e; ++iter_s)
        {
            if ( (iter_s - begin) % 10 == 0 )
                os <<  _keyid_map.key(iter_s->first) << " " << iter_s->second << "\n";
            else
                os << _keyid_map.key(iter_s->first) << " " << iter_s->second << sep;
        }
        os << "\n";
