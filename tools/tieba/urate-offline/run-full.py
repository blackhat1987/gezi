...
while(True):
	...
	if now != pre and now_hour >= 2  and now_hour <= 11:
		print 'run %s'%now 
	...
	if now != pre_train and now_hour == 12:
		pre_train = now
	...
	if now != pre2 and now_hour == 16:
		...
		os.system('sh predict.sh %s'%(last_day))
		print '-----------after predict'
		os.system('./write-db2.py ./uinfo.%s.txt 0.995 10000 ./uinfo.%s.txt.bak'%(last_day, last_day))
