...
out_train.write('#phrase\tlabel\tfreq\tpmi\trinfo\tlinfo\n')
for (phrase, pinfo) in phrases_info.items():
	if 'freq' in pinfo and 'label' in pinfo:
		label = pinfo['label']
		out_train.write('_%s\t%d\t%d\t%f\t%f\t%f\n'%(phrase.encode('gbk', 'ignore'), label, pinfo['freq'], pinfo['pmi'], pinfo['rinfo'], pinfo['linfo']))

out = open('phrase.feature.txt', 'w')
out.write('#phrase\tlabel\tfreq\tpmi\trinfo\tlinfo\n')
label = 0
for (phrase, pinfo) in phrases_info.items():
	if 'freq' in pinfo and 'label' not in pinfo:
		out.write('_%s\t%d\t%d\t%f\t%f\t%f\n'%(phrase.encode('gbk', 'ignore'), label, pinfo['freq'], pinfo['pmi'], pinfo['rinfo'], pinfo['linfo']))
